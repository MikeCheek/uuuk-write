...
const ButtonTop = ({ onClick, text, onClickScrolled, textScrolled }: { onClick: () => void, text: string, onClickScrolled?: () => void, textScrolled?: string }) => {
  ...
  return (
    <button
      onClick={handleClick}
      className={`fixed cursor-none z-[100] top-4 right-4 rounded-lg border px-4 py-2 text-base font-bold uppercase tracking-wide transition-all duration-300 ease-in-out hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-[#0b1122] focus:ring-[#f97516] ${scrolled ? 'border-[#9ad0ff]/45 bg-[#9ad0ff]/10 text-[#d8ecff]' : 'border-[#ffb170]/35 bg-gradient-to-r from-[#f97516] to-[#ff9d57] text-[#0b1122] shadow-[0_10px_24px_rgba(249,117,22,0.26)]'
        }`}
